...
public class NBTStructureGenerator implements IWorldGenerator {
	...
	private void generateStructureAt(String name, WorldServer world, Random random, BlockPos pos) {
		...
		pos = pos.add(random.nextInt(16), 0, random.nextInt(16));
		pos = pos.add(this.settings.getOffsetPos().getX(), this.settings.getOffsetPos().getY(), this.settings.getOffsetPos().getZ());
		...
		if(template != null) {
			...
			BlockPos centerPos = pos.add(Template.transformedBlockPos(settings, new BlockPos(size.getX() / 2, 0, size.getZ() / 2)));
			int count_down = 1;
			while(world.isAirBlock(centerPos.down(count_down))) {
				centerPos = centerPos.down(count_down);
				count_down++;
			}
			pos = pos.down(count_down);

			if(this.settings.getBlockOnGen() != null) {
				IBlockState block = ParseFiles.getBlock(this.settings.getBlockOnGen());
				if(!(world.getBlockState(centerPos.down()).getBlock().equals(block.getBlock())))
					return;
			}

			//System.out.println(centerPos + " | " + pos + " | " + world.getBlockState(centerPos.down()));
